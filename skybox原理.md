...
2. `parseEyesDat` 按 Eyes 布局解析（`int32 count` + 每星 23 字节），做黄道→J2000 旋转与颜色归一；
3. `billboards.add({ position, image, color, scale, disableDepthTestDistance: Infinity })`（尺寸由 `absMag` 与真实距离推出）；
4. 每帧更新 `collection.modelMatrix`（ICRF→Fixed）。

## 3. 惯性锁定：Inertial → Fixed

星表位置按**惯性系**（J2000 / 近似 ICRF）存放。Cesium 场景默认在**地固系**（ITRF/Fixed）里渲染地球。
...
```
assets/eyes-stars/
  stars.0.dat … stars.5.dat
  galaxies.0.dat
  README.md          ← NASA Eyes 二进制布局说明
js/starfield.js      ← 解析 + BillboardCollection + 惯性锁
index.html           ← CDN Cesium 入口
```

星表自 ASTROX.SolarViewer 的 `assets/eyes-stars/` 同步。二进制细节以该目录 README 为准。
...
- **Billboard 星表**：每颗星一个精灵；位置/颜色来自 Eyes 目录，大小由 `absMag` 与距离计算；用 **ICRF→Fixed** 显式锁定惯性天球。
